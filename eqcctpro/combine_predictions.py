"""Combine single prediction outputs into a single CSV file.

This is a script version of first section in
``eqcctpro/notebooks/compile_results.ipynb``. It expects a directory containing predicted P/S picks in CSV format from a deep learning model such as EQCCT or EQT.
After specifying both the model type and the waveform type, it will combine all the CSV files into a single CSV file, while also extracting metadata (time windows, station, network). 
The output CSV will contain all the predicted picks along with their associated metadata.
"""
...
waveform_type = "explosions"

SCRIPT_DIR = Path(__file__).resolve().parent

# base path: where to gather the outputs from
base_path = SCRIPT_DIR / "results" / "csv"
...
# With EQT, the .mseed filename is not available, but network and station appear to be correct in the predictions .csv
else:
...
output_file = csv_output_path / csv_output_name
...
print(f"Number of empty prediction rows kept: {num_negatives}")
